...
use gd_backend::auth::route::{add_user, create_user, edit, login};
use gd_backend::config::{get_custom_figment, init_my_config};
use gd_backend::db::init_gd_data;
use gd_backend::project::route::{add_workmate_to_project, delete_project, get_participants, get_project, set_project};
...
fn rocket()-> _ {
    rocket::custom(get_custom_figment())
        ...
        .mount("/user",routes![add_user,create_user,login,edit])
        .mount("/project",routes![set_project,get_project,delete_project,
            add_workmate_to_project,get_participants])
